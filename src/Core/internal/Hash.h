...
namespace ge::Core
{
	namespace internal
	{
		...
		constexpr uint32_t crc32impl(uint32_t crc, const char *buf, size_t left)
		{
			return left ? crc32impl(crc_table[(crc ^ *buf) & 0xFF] ^ (crc >> 8), buf + 1, left - 1) : ~crc;
		}
	}

	constexpr uint32_t crc32(const char *buf, const size_t len)
	{
		return internal::crc32impl(0xFFFFFFFF, buf, len);
	}

	// A Fast, Minimal Memory, Consistent Hash Algorithm - John Lamping, Eric Veach (2014)
	...
	{
		constexpr uint32_t operator"" _crc32(const char *str, size_t len)
		{
